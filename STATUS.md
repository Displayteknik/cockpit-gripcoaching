...
| 1.2/1.3 **Övriga browser-skriv-tabeller** | **KVAR (väg A refaktor, STEG 4)** | `hm_pages/blog/vehicles/art*` skrivs av dashboard-editorer via anon → kräver flytt till server-routes. `hm_leads`/`hm_brand_profile` = server-only batch 2 |
