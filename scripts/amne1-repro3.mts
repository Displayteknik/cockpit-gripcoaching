// ÄMNE-1, orsaksanalys steg 3: den mest sannolika mekanismen.
//
// `topic` (Ämne, steg 1) och headline/body (Text på bilden, steg 4) är TVÅ SKILDA
// state-fält i StudioMaker.tsx. `applySuggestion` (rad 802) sätter headline1/headline2/body
// när användaren väljer en idé — men rör ALDRIG `topic`. Skriver användaren själv ett ämne
// i steg 1, väljer sedan EN ANNAN idé (eller skriver om steg 4 för hand, "Skapa åt mig",
// eller öppnar ett tidigare utkast), står den gamla ämnestexten kvar OSYNKAD.
//
// I suggest-caption byggs underlaget som "Ämne: X." FÖRST, före "Rubrik på bilden:" och
// "Text på bilden:". Testet här: vad händer när Ämne pekar på ETT ämne (kvarlämnat från en
// tidigare idé om sensommar/skyltning) och Rubrik/Text på bilden pekar på ETT ANNAT
// (menyskärmen han faktiskt skapade bild för)?
...
// Kvarlämnat ämne från EN TIDIGARE idé samma session — aldrig rensat av applySuggestion.
...
// EXAKT samma ordning som suggest-caption/route.ts bygger underlaget: Ämne FÖRST.
const bygg = await byggTextPrompt({
  clientId,
  syfte: "caption",
  kanal: "instagram",
  uppdrag,
  underlag: [`Ämne: ${STALE_TOPIC}.`, contentBlock, "\nSkriv captionen nu — strukturerad enligt reglerna."].join("\n"),
  anvandarText: STALE_TOPIC,
});

console.log("=== USER-MEDDELANDET (Ämne pekar på EN sak, Rubrik/Text på EN ANNAN) ===\n");
...
console.log("=== TRE VARIANTER, MED KVARLÄMNAT ÄMNE ===\n");
for (let i = 0; i < 3; i++) {
