...
export default function AnalyticsDashboard() {
  ...
  /**
   * Rapporten som utskriftsfärdigt dokument. Delad med kundvyn (/k/seo) — samma
   * `byggRapportHtml`, alltså exakt samma PDF oavsett vem som sparar den.
   *
   * ★ ERSATTE 170 RADER HANDBYGGD jsPDF-LAYOUT. Den vägen var en återvändsgränd: jsPDF:s
   *   inbyggda typsnitt klarar bara Latin-1, så koden fick mappa bort tecken den inte kunde
   *   rita — "✅" blev "OK", "→" blev "->", och allt utanför mappningen ströks. En rapport
   *   till en svensk kund ska inte behöva översätta bort sina egna tecken.
   *
   *   Webbläsarens utskrift ger i stället vektor-PDF med sökbar text, riktiga sidbrytningar,
   *   svenska tecken och figurerna med — utan något beroende alls.
   */
  function downloadPdf() {
    if (!reportText) return;
    const html = byggRapportHtml(reportText, {
      klientNamn: data?.client?.name ?? "Klient",
