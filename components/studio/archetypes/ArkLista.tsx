  ...
  return (
    <div id="studio-canvas" style={{ width: w, height: h, position: "relative", overflow: "hidden", display: "flex", flexDirection: "column", fontFamily: `${brand.fonts.body}, sans-serif`, background: c.paper }}>
      <div style={{ padding: "56px 64px 20px" }}>
        <div style={{ fontFamily: `${brand.fonts.headline}, sans-serif`, fontWeight: 800, color: c.primary, fontSize: 68, lineHeight: 1.02, textTransform: "uppercase", letterSpacing: -0.5 }}>{payload.headline1}</div>
        {payload.headline2 ? <div style={{ fontFamily: `${brand.fonts.body}, sans-serif`, fontWeight: 600, color: c.ink, fontSize: 34, marginTop: 18 }}>{payload.headline2}</div> : null}
      </div>
        ...
        {items.map((item, i) => (
          <div key={i}>
            <div style={{ display: "flex", alignItems: "center", gap: 24, padding: "18px 0" }}>
              <div style={{ width: 68, height: 68, borderRadius: brand.elements.shapes.style === "sharp" ? 8 : 999, background: c.accent, color: numInk, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: `${brand.fonts.headline}, sans-serif`, fontWeight: 800, fontSize: 34, flexShrink: 0 }}>{i + 1}</div>
              <div style={{ fontFamily: `${brand.fonts.body}, sans-serif`, fontWeight: 600, color: c.ink, fontSize: 36, lineHeight: 1.2 }}>{item}</div>
            </div>
