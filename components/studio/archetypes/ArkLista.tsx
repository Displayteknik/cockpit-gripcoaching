  ...
  return (
    <div id="studio-canvas" style={{ width: w, height: h, position: "relative", overflow: "hidden", display: "flex", flexDirection: "column", fontFamily: `${brand.fonts.body}, sans-serif`, background: c.paper }}>
      <div style={{ padding: "56px 64px 20px" }}>
        <div style={{ fontFamily: `${brand.fonts.headline}, sans-serif`, fontWeight: 800, color: c.primary, fontSize: fs(68, payload), lineHeight: 1.02, textTransform: "uppercase", letterSpacing: -0.5 }}>{payload.headline1}</div>
        ...
        {items.map((item, i) => (
          <div key={i}>
            <div style={{ display: "flex", alignItems: "center", gap: 24, padding: "18px 0" }}>
              ...
              <div style={{ fontFamily: `${brand.fonts.body}, sans-serif`, fontWeight: 600, color: c.ink, fontSize: fs(36, payload), lineHeight: 1.2 }}>{item}</div>
            </div>
