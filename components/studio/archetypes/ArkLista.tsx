  ...
  return (
    <div id="studio-canvas" style={{ width: w, height: h, position: "relative", overflow: "hidden", display: "flex", flexDirection: "column", fontFamily: `${brand.fonts.body}, sans-serif`, background: c.paper }}>
      <div style={{ padding: "56px 64px 20px" }}>
        <div style={{ fontFamily: `${brand.fonts.headline}, sans-serif`, fontWeight: 800, color: hlColor(c.primary, payload), fontSize: fs(68, payload), lineHeight: 1.02, textTransform: "uppercase", letterSpacing: -0.5 }}><span data-edit="headline1">{payload.headline1}</span></div>
        {payload.headline2 ? <div style={{ fontFamily: `${brand.fonts.body}, sans-serif`, fontWeight: 600, color: c.ink, fontSize: fs(34, payload), marginTop: 18 }}><span data-edit="headline2">{payload.headline2}</span></div> : null}
      </div>
        ...
        {items.map((item, i) => (
          <div key={i}>
            <div style={{ display: "flex", alignItems: "center", gap: 24, padding: "18px 0" }}>
              ...
              <div style={{ fontFamily: `${brand.fonts.body}, sans-serif`, fontWeight: 600, color: bodyColor(c.ink, payload), fontSize: fs(36, payload), lineHeight: 1.2 }}><span data-edit={`list-${i}`}>{item}</span></div>
            </div>
