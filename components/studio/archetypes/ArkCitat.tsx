  ...
  return (
    <div id="studio-canvas" style={{ width: w, height: h, position: "relative", overflow: "hidden", display: "flex", flexDirection: "column", fontFamily: `${brand.fonts.body}, sans-serif`, fontVariantNumeric: "lining-nums", background: bg }}>
      <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", padding: "0 84px", position: "relative" }}>
        ...
        {payload.headline1 ? (
          <div style={{ fontFamily: `${brand.fonts.body}, sans-serif`, fontWeight: 700, textTransform: "uppercase", letterSpacing: 2, color: c.primary, fontSize: fs(22, payload, "h1"), marginBottom: 10 }}><span data-edit="headline1" style={textPlate(payload)}>{payload.headline1}</span></div>
        ) : null}
        <div style={{ fontFamily: font(brand.fonts.headline, payload), fontWeight: 700, color: bodyColor(c.ink, payload), fontSize: fs(52, payload, "body"), lineHeight: lh(1.28, payload) }}><span data-edit="body" style={textPlate(payload)}>{payload.body}</span></div>
        {payload.headline2 ? (
