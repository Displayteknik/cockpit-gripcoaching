  ...
  return (
    <div id="studio-canvas" style={{ width: w, height: h, position: "relative", overflow: "hidden", display: "flex", flexDirection: "column", fontFamily: `${brand.fonts.body}, sans-serif`, background: c.paper }}>
      <div style={{ padding: "44px 56px 0", textAlign: "center" }}>
        <div style={{ fontFamily: `${brand.fonts.headline}, sans-serif`, fontWeight: 800, color: c.primary, fontSize: 60, lineHeight: 1.0, textTransform: "uppercase", letterSpacing: -0.5 }}>{payload.headline1}</div>
        {payload.headline2 ? <div style={{ fontFamily: `${brand.fonts.body}, sans-serif`, fontWeight: 600, color: c.ink, fontSize: 36, marginTop: 16 }}>{payload.headline2}</div> : null}
      </div>
      ...
      <div style={{ position: "relative", flex: 1, margin: "22px 0 0", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ position: "absolute", inset: 0, overflow: "hidden" }}>
          {payload.imageUrl ? (
            ...
            <img src={payload.imageUrl} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: `center ${payload.imageFocusY}%` }} />
        ...
        {payload.badge.enabled ? (
          shape === "starburst" ? (
            ...
            <div style={{ position: "relative", width: 340, height: 340, borderRadius: shape === "tag" ? 28 : 999, background: c.accent, color: badgeInk, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", boxShadow: "0 18px 50px rgba(0,0,0,0.18)" }}>
              <span style={{ fontFamily: `${brand.fonts.body}, sans-serif`, fontWeight: 700, fontSize: 34, textTransform: "uppercase", letterSpacing: 1 }}>{payload.badge.line1}</span>
              <span style={{ fontFamily: `${brand.fonts.headline}, sans-serif`, fontWeight: 800, fontSize: 88, lineHeight: 0.95 }}>{payload.badge.line2}</span>
            </div>
          )
        ) : payload.body ? (
          <div style={{ position: "relative", maxWidth: 780, textAlign: "center", fontFamily: `${brand.fonts.headline}, sans-serif`, fontWeight: 800, color: onPrimary, fontSize: 64, lineHeight: 1.05, padding: "0 40px" }}>{payload.body}</div>
        ) : null}
      </div>
