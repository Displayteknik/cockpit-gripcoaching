  ...
  return (
    <div id="studio-canvas" style={{ width: w, height: h, position: "relative", overflow: "hidden", display: "flex", flexDirection: "column", fontFamily: `${brand.fonts.body}, sans-serif`, background: c.paper }}>
      <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", padding: "0 80px" }}>
        ...
        {payload.headline1 ? (
          <div style={{ fontFamily: `${brand.fonts.body}, sans-serif`, fontWeight: 700, textTransform: "uppercase", letterSpacing: 2.5, color: c.primary, fontSize: 24, marginBottom: 18 }}>{payload.headline1}</div>
        ) : null}
        {payload.headline2 ? (
          <div style={{ fontFamily: `${brand.fonts.headline}, sans-serif`, fontWeight: 800, color: c.ink, fontSize: 58, lineHeight: 1.08, letterSpacing: -0.5, marginBottom: 26 }}>{payload.headline2}</div>
        ) : null}
        {payload.body ? (
          <div style={{ fontFamily: `${brand.fonts.body}, sans-serif`, fontWeight: 400, color: c.ink, opacity: 0.86, fontSize: 36, lineHeight: 1.45, maxWidth: 820 }}>{payload.body}</div>
        ) : null}
      </div>
