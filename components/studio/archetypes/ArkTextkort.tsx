  ...
  return (
    <div id="studio-canvas" style={{ width: w, height: h, position: "relative", overflow: "hidden", display: "flex", flexDirection: "column", fontFamily: `${brand.fonts.body}, sans-serif`, fontVariantNumeric: "lining-nums", background: c.paper }}>
      <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", padding: "0 80px" }}>
        ...
        {payload.headline1 ? (
          <div data-drag="h1" style={{ fontFamily: `${brand.fonts.body}, sans-serif`, fontWeight: 700, textTransform: "uppercase", letterSpacing: 2.5, color: c.primary, fontSize: fs(24, payload, "h1"), marginBottom: 18, ...dragPos(payload, "h1") }}><span data-edit="headline1" style={textPlate(payload)}>{payload.headline1}</span></div>
        ) : null}
        {payload.headline2 ? (
          <div data-drag="h2" style={{ fontFamily: font(brand.fonts.headline, payload), fontWeight: 800, color: hlColor(c.ink, payload), fontSize: fs(58, payload, "h2"), lineHeight: lh(1.12, payload), letterSpacing: -0.5, marginBottom: 26, ...dragPos(payload, "h2") }}><span data-edit="headline2" style={textPlate(payload)}>{payload.headline2}</span></div>
        ) : null}
        {payload.body ? (
