...
import { fs, hlColor, bodyColor, imgPosition, imgScale, imgFit, showBrush, showBadge, font, textPlate, lh, dragPos } from "@/lib/studio/overrides";
...
export default function ArkFotoRuta({ payload, brand }: { payload: StudioPayload; brand: StudioBrand }) {
  ...
  return (
    <div id="studio-canvas" style={{ overflowWrap: "break-word", width: w, height: h, position: "relative", overflow: "hidden", background: c.paper, display: "flex", flexDirection: "column", fontFamily: `${brand.fonts.body}, sans-serif`, fontVariantNumeric: "lining-nums" }}>
      ...
      <div style={{ position: "relative", flex: 1, margin: "16px 0 0" }}>
        <div data-edit-image style={{ position: "absolute", inset: 0, overflow: "hidden" }}>
          {payload.imageUrl ? (
            ...
            <img src={payload.imageUrl} alt="" style={{ width: "100%", height: "100%", objectFit: imgFit(payload), objectPosition: imgPosition(payload), transform: `scale(${imgScale(payload)})`, display: "block" }} />
