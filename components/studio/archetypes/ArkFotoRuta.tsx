  ...
  return (
    <div id="studio-canvas" style={{ width: w, height: h, position: "relative", overflow: "hidden", background: c.paper, display: "flex", flexDirection: "column", fontFamily: `${brand.fonts.body}, sans-serif` }}>
      <div style={{ padding: "40px 50px 0", textAlign: "center" }}>
        <div style={{ fontFamily: `${brand.fonts.headline}, sans-serif`, fontWeight: 800, textTransform: "uppercase", color: hlColor(c.primary, payload), fontSize: fs(62, payload), lineHeight: 1.0, letterSpacing: -0.5 }}><span data-edit="headline1">{payload.headline1}</span></div>
        {payload.headline2 ? <div style={{ fontFamily: `${brand.fonts.headline}, sans-serif`, fontWeight: 800, color: c.ink, fontSize: fs(50, payload), lineHeight: 1.05, marginTop: 6 }}><span data-edit="headline2">{payload.headline2}</span></div> : null}
      </div>
        ...
        {payload.body ? (
          <div style={{ position: "absolute", left: 36, right: payload.badge.enabled ? 230 : 36, bottom: -18, minHeight: 150 }}>
            ...
            <div style={{ position: "relative", minHeight: 150, boxSizing: "border-box", display: "flex", alignItems: "center", padding: "34px 56px" }}>
              <div style={{ fontFamily: `${brand.fonts.body}, sans-serif`, fontWeight: 700, color: bodyColor(ink, payload), fontSize: fs(30, payload), lineHeight: 1.25 }}><span data-edit="body">{payload.body}</span></div>
            </div>
          </div>
        ) : null}
