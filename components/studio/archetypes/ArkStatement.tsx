  ...
  return (
    <div id="studio-canvas" style={{ width: w, height: h, position: "relative", overflow: "hidden", display: "flex", flexDirection: "column", fontFamily: `${brand.fonts.body}, sans-serif`, fontVariantNumeric: "lining-nums", background: c.primary }}>
      <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", padding: "0 90px" }}>
        <div style={{ fontFamily: font(brand.fonts.headline, payload), fontWeight: 800, color: hlColor(onPrimary, payload), fontSize: fs(96, payload, "h1"), lineHeight: lh(1.06, payload), letterSpacing: -1.5, textTransform: "uppercase" }}>
        ...
        {payload.headline2 ? (
          <div style={{ fontFamily: `${brand.fonts.body}, sans-serif`, fontWeight: 600, color: onPrimary, opacity: 0.92, fontSize: fs(42, payload, "h2"), lineHeight: lh(1.2, payload) }}><span data-edit="headline2" style={textPlate(payload)}>{payload.headline2}</span></div>
        ) : null}
        {payload.body ? (
          <div style={{ fontFamily: `${brand.fonts.body}, sans-serif`, fontWeight: 400, color: bodyColor(onPrimary, payload), opacity: 0.85, fontSize: fs(30, payload, "body"), lineHeight: lh(1.4, payload), marginTop: 22, maxWidth: 760 }}><span data-edit="body" style={textPlate(payload)}>{payload.body}</span></div>
        ) : null}
      </div>
