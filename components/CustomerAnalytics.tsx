...
import { computeFocusInsights, type FocusIcon } from "@/lib/dashboard-insights";

// Token → ikon, så den delade insikts-motorn kan köras både server- och clientside.
...
export default function CustomerAnalytics({ primaryColor, clientName, snippet = "" }: { primaryColor: string; clientName: string; snippet?: string }) {
  ...
  // "Att fokusera på" — konkreta förbättringstips ur kundens egen data (delad motor,
  // samma som "Att göra nu" på översikten så det aldrig driftar isär).
  const topInsights = computeFocusInsights(data).slice(0, 3);
