...
export async function buildDashboardData(clientId: string, daysRaw: number) {
  ...
  // GA4 = auktoritativ trafik/kanal/AI-data. null om ej kopplat.
  ...
  for (const r of gscRows) {
    ...
    if (pos > 0) { e.positionSum += pos * r.impressions; e.positionN += r.impressions; }
    if (r.page) e.pages.set(r.page, (e.pages.get(r.page) || 0) + r.impressions);
  }
  ...
  for (const r of gscRows) {
    ...
    e.clicks += r.clicks; e.impressions += r.impressions; e.queryCount++;
  }
  ...
  // Sökord per band — för klickbar drill-down (störst möjlighet = flest visningar först)
  ...
  // Quick wins: position 4-15 OCH impressions >= 30
  ...
  // Sessioner vs sidvisningar (samma session_id = ETT besök)
  ...
  // Trafik över tid (distinkta sessioner per dag)
  ...
  // Top paths från egen pixel
  const byPath = new Map<string, number>();
  const refMap = new Map<string, number>();
  for (const v of visitRows) {
    if (v.path) byPath.set(v.path, (byPath.get(v.path) || 0) + 1);
    if (v.referrer) {
      try { const h = new URL(v.referrer).hostname.replace(/^www\./, ""); if (h) refMap.set(h, (refMap.get(h) || 0) + 1); } catch {}
    }
  }
  ...
  try {
    ...
    if (locs.length > 1) {
      ...
      for (const q of queries) {
        ...
        if (samePagePath(ideal, q.page)) continue;
