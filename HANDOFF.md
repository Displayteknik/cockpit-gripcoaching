# HANDOFF — Cockpit, läget 2026-07-31
...
**Senaste commit:** `97947a0`. Allt nedan är pushat och i produktion om inget annat sägs. 103 enhetstester gröna, `tsc --noEmit` och `next build` rena.
...
- **REVISION-1** (REV-0-rapporten godkänd, etapperna ej startade): REV-1 felhantering → REV-2 kvitton → REV-3 tomma lägen och UI → REV-4 FunctionGuide och språk. Tre obesvarade frågor i rapporten: ordvalet rådgivare/konsult/byrå, om `hq` och `webbdata-demo` ska ut ur menyn, och om döda `SkapaInlaggMaker.tsx` (1300 rader, noll importörer) ska tas bort.
- **Parkerat, inget byggs:** automatisk omgenerering vid detekterat förbjudet klientord (i dag detektering + logg), och röstträff-åtgärderna för linkedin.

**Dataproblem, inte kodproblem:** HM Motor-tenanten `00000000-…001` har en coaching-/kundflödesprofil i databasen, inte bilhandel. Texterna blir korrekta mot profilen men fel mot etiketten. Profilen behöver uppdateras.
