...
export async function GET(req: NextRequest) {
  ...
  const tagg = (sp.get("tagg") || "").trim().toLowerCase();
  ...
  let fraga = sb
    ...
    .limit(TAK);
  if (tagg) fraga = fraga.contains("taggar", [tagg]);
  ...
  // Taggarna som faktiskt förekommer, för filtret. Räknas ur det tenanten HAR, inte ur
  // en handskriven lista — en tagg som inte finns ska inte gå att välja.
  const taggRakning = new Map<string, number>();
  for (const r of rader) for (const t of r.taggar || []) taggRakning.set(t, (taggRakning.get(t) || 0) + 1);
  const taggar = [...taggRakning.entries()]
    .sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0], "sv"))
    .map(([namn, antal]) => ({ namn, antal }));

  // Sökningen körs i minnet, efter taggfiltret. Motivering: träffen ska gälla namn,
  // företag, e-post OCH telefon samtidigt, och en or-fråga över fyra kolumner med
  // användarens fritext är precis den sortens sträng-hopfogning som blir ett injektionshål.
  const traffar = q
    ? rader.filter((r) =>
        [r.namn, r.foretag, r.epost, r.telefon].some((f) => (f || "").toLowerCase().includes(q)),
      )
    : rader;
